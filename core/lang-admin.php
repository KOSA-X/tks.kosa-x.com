  ...
  if( is_file( $config['dir_database'].'lang_'.$sLang.'.php' ) ){
    ...
    foreach( $lang as $sKey => $sValue ){
      ...
      if( !isset( $aDontDisplay[$sKey] ) ){
        ...
        $keyEsc = htmlspecialchars( $sKey, ENT_QUOTES, 'UTF-8' );

        $aReturn[$iKey] .= '<li>
          <label for="'.$keyEsc.'">'.$keyEsc.'</label>
          <input type="text" name="'.$keyEsc.'" value="'.$val.'" id="'.$keyEsc.'" size="80" />'
          .( isset( $aLangRaw[$sKey] ) ? ' = '.htmlspecialchars( $aLangRaw[$sKey], ENT_QUOTES, 'UTF-8' ) : null ).
        '</li>';
      }
    }
