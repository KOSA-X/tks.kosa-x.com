...
function customCheckbox(){
  // OPTYMALIZACJA: Cache selektora jQuery, użyj chaining
  $('.custom input[type=checkbox] ~ label').each( function(){
    var $label = $( this ).siblings( '.label' );
    $label.html( $( this ).html() ).click(
      { oObj:this },
      function(e){
...
function displayTab( oObj, bInit ){
  ...
  if( bInit && window.location.hash.replace( '#', '' ) != '' ){
    ...
    sBlock = $( oObj ).attr( 'id' );
  if( $('#'+sBlock).length > 0 ){
    // OPTYMALIZACJA: Cache selektorów, unikaj powtarzania zapytań DOM
    var $tabs = $( '.tabs li' );
    $tabs.removeClass( 'selected' );
    $tabs.filter( '#'+sBlock ).addClass( 'selected' );
    $( '.forms' ).hide();
    $( '#tab-'+sBlock ).show();
...
function filesFromServerEvents( ){
  // OPTYMALIZACJA: Użyj .off() przed .on() aby uniknąć wielokrotnego podpinania handlerów
  $( '#files-dir-table' ).off( 'click', 'td.select input[type=checkbox]' ).on( 'click', 'td.select input[type=checkbox]', function(){displayFilesOptions( this );} );
  $( '#files-dir-table' ).fixedfiles();
  $( '.forms.files' ).off( 'click', '.clear-default' ).on( 'click', '.clear-default', function(e){ e.preventDefault(); clearThumbDefault( this ); } );
}
...
/**
 * Paginacja + wyszukiwarka listy plików z serwera (zakładka "Dodaj pliki").
 * Podmienia cały .files-dir-container przez replaceWith (bez zagnieżdżania),
 * odpowiedź generuje FilesAdmin::listFilesInDir (ajax-files-in-dir).
 */
function loadFilesDir( iSite, sSearch ){
  var $c = $( '#files-dir .files-dir-container' ).first();
  if( !$c.length ){
    return;
  }

  var sUrl = aQuick['sPhpSelf']+'?p=ajax-files-in-dir'
    +'&iSite='+encodeURIComponent( iSite || 1 )
  ...
  $.get( sUrl, function( sHtml ){
    var $new = $( sHtml );
    $c.replaceWith( $new );
    // klon nagłówka z fixedfiles() dotyczył podmienionej tabeli — usuń,
    // inaczej nagłówek się dubluje; filesFromServerEvents zrobi świeży
    $( '.fixed-files' ).remove( );
    // przywróć fokus i kursor w wyszukiwarce (pole przyszło z nowym HTML-em)
    var oSearch = document.getElementById( 'files-dir-search' );
    if( oSearch && sSearch ){
      oSearch.focus( );
      oSearch.setSelectionRange( oSearch.value.length, oSearch.value.length );
    }
    filesFromServerEvents( );
  });
}

// zdarzenia delegowane na document — kontener jest podmieniany w całości
$( document ).on( 'click', '.files-dir-page', function( e ){
  e.preventDefault( );
  // inputy zaznaczonych plików żyją w wierszach bieżącej strony —
  // zmiana strony listy je czyści, więc uprzedzamy
  if( $( '#files-dir-table td.select input:checked' ).length
      && !confirm( 'Masz zaznaczone pliki na tej stronie listy — zmiana strony wyczyści zaznaczenie. Kontynuować?' ) ){
    return;
  }
  loadFilesDir( $( this ).attr( 'data-site' ), $( '#files-dir-search' ).val( ) );
});

var iFilesDirSearchTimer = null;
$( document ).on( 'input', '#files-dir-search', function( ){
  var sVal = $( this ).val( );
  clearTimeout( iFilesDirSearchTimer );
  iFilesDirSearchTimer = setTimeout( function( ){
    loadFilesDir( 1, sVal );
  }, 400 );
});
...
(function($){
  // OPTYMALIZACJA: Funkcja throttle do ograniczenia częstotliwości wywołań
  ...
  $.fn.fixedfiles = function(){ return this.each(function(){
    ...
    // OPTYMALIZACJA: Throttle scroll i resize handlerów (max 1 wywołanie na 100ms)
    $(window).scroll(throttle(scrollHeader, 100));
    $(window).resize(throttle(fixColumnsSize, 100));
...
/**
 * Ochrona CSRF panelu admina — dołącza token do wszystkich żądań
 * zmieniających stan (formularze POST, linki usuwania, AJAX jQuery).
 * Token pochodzi z aQuick['sTokenCsrf'] (renderowany serwerowo w _header.php).
 */
function initCsrfProtection(){
  var sToken = ( typeof aQuick !== 'undefined' && aQuick['sTokenCsrf'] ) ? aQuick['sTokenCsrf'] : '';
  if( !sToken )
    return;

  // 1) Ukryte pole w każdym formularzu (POST i GET)
  $( 'form' ).each( function(){
    var $form = $( this );
    if( $form.find( 'input[name="sTokenCsrf"]' ).length === 0 ){
      $( '<input>', { type: 'hidden', name: 'sTokenCsrf', value: sToken } ).appendTo( $form );
    }
  });

  // 2) Token w linkach usuwających (GET: iItemDelete / sItemDelete)
  $( 'a[href*="iItemDelete="], a[href*="sItemDelete="]' ).each( function(){
    var sHref = $( this ).attr( 'href' );
    if( sHref && sHref.indexOf( 'sTokenCsrf=' ) === -1 ){
      $( this ).attr( 'href', sHref + ( sHref.indexOf( '?' ) === -1 ? '?' : '&' ) + 'sTokenCsrf=' + encodeURIComponent( sToken ) );
    }
  });

  // 3) Nagłówek dla wszystkich żądań AJAX jQuery (POST oraz mutujące GET)
  if( $.ajaxSetup ){
    $.ajaxSetup({ headers: { 'X-CSRF-Token': sToken } });
  }
}
