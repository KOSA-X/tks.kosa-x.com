...
function listSlidersAdmin( ){
  ...
  while( $aData = $oQuery->fetch( PDO::FETCH_ASSOC ) ){
    $sFileNameEsc = htmlspecialchars( (string) $aData['sFileName'], ENT_QUOTES, 'UTF-8' );
    $sTitleEsc    = htmlspecialchars( (string) $aData['sTitle'], ENT_QUOTES, 'UTF-8' );
    $content .= '<tr class="l'.( ( ++$i % 2 ) ? 0: 1 ).'"><td class="image"><div class="flex"><a href="?p=sliders-form&amp;iSlider='.$aData['iSlider'].'" style="width:200px; flex-basis:200px"><img src="files/'.$sFileNameEsc.'" alt=""  /></a><div class="more-info ml-2"><strong>'.$sTitleEsc.'</strong><br><p class="filename muted mt-0"><small>'.$sFileNameEsc.'</small></p></div></td><td class="position"><input type="text" name="aPositions['.$aData['iSlider'].']" value="'.$aData['iPosition'].'" class="numeric" size="3" maxlength="4" /></td><td class="options">
    <a href="?p=sliders&amp;iItemDelete='.$aData['iSlider'].'" onclick="return del( )" class="options_button danger"><img src="templates/admin/img/delete.svg"></a></td></tr>';
  } // end while
...
function listSlidersFilesAdmin( $sFileSelect = null ){ 
  ...
  while( $aData = $oQuery->fetch( PDO::FETCH_ASSOC ) ){ 
    $sFileNameEsc = htmlspecialchars( (string) $aData['sFileName'], ENT_QUOTES, 'UTF-8' );
    $content .= '<option value="'.$sFileNameEsc.'"'.( ( isset( $sFileSelect ) && $sFileSelect == $aData['sFileName'] ) ? ' selected="selected"' : null ).'>'.$sFileNameEsc.'</option>';
  } // end while 
  return $content; 
